...
-- Update with image URLs from Supabase (run this after upload-images.ts outputs the mapping)
-- Example mapping (replace with actual URLs from upload output):
UPDATE public.products SET images = ARRAY['https://your-project.supabase.co/storage/v1/object/public/products/accra-heat-oversized-hoodie/Accra%20HEAT%20Oversized%20Hoodie.jpg'] WHERE slug = 'accra-heat-oversized-hoodie';